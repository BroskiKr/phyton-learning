...
def read_posts(
    ...
    if user["last_name"] == "admin":
        ...
        total_count = db.query(models.Post).filter(models.Post.owner_id == user_data.id).count()
        if limit > 0:
            posts = (
                db.query(models.Post).filter(models.Post.owner_id == user_data.id).offset((page - 1) * limit).limit(limit).all()
            )
        else:
            posts = (
...
@router.post(
    "/autogenerate", status_code=status.HTTP_201_CREATED,response_model=list[schemas.PostResponse]
)
def autogenerate_posts(db: Session = Depends(get_db),user_data: schemas.TokenData = Depends(oauth2.get_current_user)):

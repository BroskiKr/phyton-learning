...
def test_create_user(auth_header):
    ...
    assert created_user['last_name'] == "Test1" and utils.verify('test',created_user['password'])
...
def test_get_users(auth_header):
    ...
    assert test1_user['email'] == "test1@gmail.com" and test1_user['last_name'] == "Test1"
...
def test_change_user(auth_header):
    ...
    assert res.status_code == 200
    assert updated_user['last_name'] == "Test2" and utils.verify('test1',updated_user['password'])
